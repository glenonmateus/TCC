...
Quando se fala em segurança de sistemas computacionais, logo vem a mente da maioria dos usuários da rede, roubo de número de cartões de crédito, \textit{hackers} danificando páginas e aplicações \textit{Web} e ataques de negação de serviço. Também vem a imagem dos \textit{malwares}, como vírus, cavalos de tróia e \textit{worms}. Esses possuem maior visibilidade pois representam uma parte significativa das ameaças existentes na Internet.
...
\item \textbf{Incidente de segurança}: qualquer evento oposto a segurança; por exemplo, ataques de negação de serviços (\textit{Denial of Service} - DoS), roubo de informações, vazamento e obtenção de acesso não autorizado a informações;
 \item \textbf{Ativo}: qualquer dado, informação e equipamento que tenha valor para a organização e para seus negócios. Alguns exemplo: banco de dados, softwares, equipamentos (computadores e notebooks), servidores, elementos de redes (roteadores, switches, entre outros), pessoas, processos e serviços;
...
O conceito mais básico e considerado o pilar de toda a área de segurança corresponde à sigla CID (Confidencialidade, Integridade e Disponibilidade), de modo que um incidente de segurança é caracterizado quando uma dessas áreas é afetada \cite{seg-redes-sistemas}. Abaixo será detalhado cada item.
...
Além dos conceito listados, a literatura considera mais alguns conceitos auxiliares:
...
Além disso, pode-se dividir os ataques em quatro categorias, que são \cite{sistemasids:joao}: 
...
Um problema muito comum é o desenvolvimento de aplicações \textit{web} sem nenhuma preocupação com segurança, podendo comprometer, não somente o serviço, mas também, em casos mais extremos, o servidor inteiro. Para tal, o atacante pode usar vários artifícios, os mais conhecidos são \textit{sql injection} e \textit{cross-site script}.
...
\item \textbf{\textit{scanner} de vulnerabilidades conhecidas}: Um vez determinados os serviços que uma máquina disponibilizada na rede entra em cena o \textit{scanner} de vulnerabilidade. A ideia é checar, através de uma lista de falhas conhecidas, se o sistema está ou não executando um serviço com problemas \cite{univhacker}. 
\end{alineas}

Normalmente, essas ferramentas funcionam em três estágios \cite{avaliacao:tania}:

\begin{alineas}
\item \textbf{Configuração}: aqui será definido o endereço IP do alvo ou a URL (\textit{Uniform Resource Locator}) da aplicação Web e demais parâmetros, como, por exemplo, utilização de \textit{proxy}.
...
\begin{figure}[!htb]
 \centering
 \caption{Estatísticas de Desfiguração de Páginas}
...
\begin{figure}[!htb]
 ...
 \begin{minipage}{0.4\textwidth}
  \centering
  \caption{Arquitetura \textit{Remote Mode} do \textit{framework} Pytbull} \label{fig:pytbull-remote}
  \includegraphics[scale=.32]{arquitetura_pytbull_remote-modificado.png}
  \legend{Adaptado de \cite{pytbull}}
\end{minipage}
\hfill
\begin{minipage}{0.4\textwidth}
\centering
\caption{Arquitetura \textit{IPS mode} do \textit{framework} Pytbull} \label{fig:pytbull-ips}
\includegraphics[scale=.32]{arquitetura_pytbull_ips-modificado.png}
\legend{Adaptado de \cite{pytbull}}
\end{minipage}
\end{figure}
...
\begin{minipage}{0.4\textwidth}
\centering
\caption{Arquitetura \textit{IPS mode with attacked server in dmz} do \textit{framework} Pytbull} \label{fig:pytbull-ips-dmz}
\includegraphics[scale=.34]{arquitetura_pytbull_ips_dmz-modificado.png}
\legend{Adaptado de \cite{pytbull}}
\end{minipage}
\hfill
\begin{minipage}{0.4\textwidth}
\centering
\caption{Arquitetura \textit{IDS mode with attacked server in dmz} do \textit{framework} Pytbull} \label{fig:pytbull-ids-dmz}
\includegraphics[scale=.34]{arquitetura_pytbull_ids_dmz-modificado.png}
\legend{Adaptado de \cite{pytbull}}
\end{minipage}
\end{figure}
...
\includegraphics[scale=.6]{openvas-modificado.png}
\legend{Adaptado de \cite{openvas}}
\label{fig:openvas}
\end{figure}
