...
O resultado do Nmap é uma tabela de portas e seus estados. As portas podem assumir os estados aberto (\textit{open}), filtrado (\textit{filtered}), fechado (\textit{closed}) ou não-filtrado (\textit{unfilterd}). As portas classificadas como \textit{open} significa que existe alguma aplicação escutando conexões. No estado \textit{filtered}, há um obstáculo na rede (firewall) que impossibilita que o Nmap determine se a porta está aberta ou fechada. As portas \textit{closed} não possuem uma aplicação escutando nelas. Por fim, são classificadas como \textit{unfilterd} quando elas respondem porém o Nmap não consegue determinar se estão fechadas ou abertas \cite{nmap}. A Figura \ref{fig:nmap} mostra um exemplo de saída do Nmap.

\begin{figure}[!htp]
 \centering
 \includegraphics[scale=.7]{nmap.png}
